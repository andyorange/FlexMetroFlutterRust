...
use crate::api::fm_ticker_base::{FMSectionTimer, BeatEvent, BeatType, AsyncTimer, FMTickerBase};
...
/// Beat information sent to subscribers for each beat/subbeat
#[derive(Debug, Clone)]
pub struct BeatInfo {
    /// Type of beat (major, medium, minor)
    pub beat_type: BeatType,
    /// Current bar metric (e.g., 4/4, 6/8)
    pub current_metric: (i32, i32),
    /// Current beat position in the bar (1-based, e.g., 1-6 in a 6/8 bar)
    pub beat_position_in_bar: usize,
    /// Current subbeat position within the beat (1-based)
    pub subbeat_position_in_beat: usize,
    /// Metric of the next bar from the tempo sequence
    pub next_bar_metric: Option<(i32, i32)>,
...
/// Composition information sent to subscribers when composition context changes
#[derive(Debug, Clone)]
...
#[derive(Debug, Clone)]
pub enum TimerEvent {
    /// Beat/subbeat event with musical timing information
    Beat(BeatInfo),
    /// Composition context change event
    CompositionContext(CompositionInfo),
...
pub struct TickerPublisher {
    ...
    last_composition_info: Arc<Mutex<Option<CompositionInfo>>>,
}
...
impl TickerPublisher {
    ...
    fn handle_beat_event(
        ...
        *current_bar_number.lock().unwrap() = beat_event.bar_index + 1;
        ...
        // Create beat info (we'll need access to the tempo sequence to get next bar metric)
        let beat_info = BeatInfo {
            beat_type: beat_event.beat_type,
            current_metric: (beat_event.nom, beat_event.denom),
            beat_position_in_bar: beat_event.beat_in_bar + 1, // Convert to 1-based
            subbeat_position_in_beat: beat_event.subbeat_in_beat + 1, // Convert to 1-based
            next_bar_metric: None, // TODO: Implement next bar lookup
            tempo_bpm: beat_event.tempo_bpm,
            bar_number: beat_event.bar_index + 1, // Convert to 1-based
            time_offset_ms: beat_event.time_offset_ms,
        };
        
        Self::notify_subscribers(subscribers, TimerEvent::Beat(beat_info));
    }
    ...
    fn send_composition_context(
        ...
        if let Some(bar_refs) = bar_ref.get_bar_objects(bar_number) {
            ...
            // Check if composition info has changed
            let mut last_info_guard = last_composition_info.lock().unwrap();
            let should_send = match last_info_guard.as_ref() {
                None => true,
                Some(last_info) => {
                    last_info.composer_name != composition_info.composer_name ||
                    last_info.composition_name != composition_info.composition_name ||
                    last_info.movement_number != composition_info.movement_number ||
                    last_info.movement_name != composition_info.movement_name ||
                    last_info.section_name != composition_info.section_name
                }
            };
            
            if should_send {
                *last_info_guard = Some(composition_info.clone());
                drop(last_info_guard);
                Self::notify_subscribers(subscribers, TimerEvent::CompositionContext(composition_info));
            }
        }
    }
